...
class MP4Atom:
    ...
    @staticmethod
    def init_from_chunk(bytes_or_path_string : Union[bytes,str], head_chunk_max_size = 50000):
      ...
      """
      if type(bytes_or_path_string) == bytes:
        return MP4Atom(bytes_or_path_string, is_root = True)        
      if type(bytes_or_path_string) == str:
        with open(bytes_or_path_string,'rb') as f:
          data = f.read(head_chunk_max_size)
        return MP4Atom(data, is_root = True)
